...
rawImage=varargin{1};
isMultispectral = ~ismatrix(rawImage);

if (isMultispectral)
    rawImage = permute(rawImage, [2,3,1]);
    [~,~,bands] = size(rawImage); 
else
    bands = 1;    
end

mResult = cell(bands,1);

for k = 1:bands
    image = squeeze(rawImage(:,:,k));
    ...
    d_C = double(C);
    if (k < bands)
        Cnext = squeeze(rawImage(origy:origy+dy,origx:origx+dx, k+1));
    end
    if (k > 1)
        Cprev = squeeze(rawImage(origy:origy+dy,origx:origx+dx, k-1));
    end
    ...
    if (isMultispectral && k > 1 && k < bands)
        result_s = (Cprev >= C) * 2^0 + (Cnext >= C) * 2^1;
    elseif (isMultispectral && k == 1)
        Cprev = C - (Cnext - C);
        result_s = (Cprev >= C) * 2^0 + (Cnext >= C) * 2^1;
    elseif (isMultispectral && k == bands)
        Cnext = C + (C - Cprev);
        result_s = (Cprev >= C) * 2^0 + (Cnext >= C) * 2^1;
    end
    
    mResult{k} = result + (neighbors + 1) * result_s;
    
end

if (isMultispectral) 
    result = cell2mat(mResult);
    bins = bins * 2^2;
else
    result = mResult{1};
end
