...
if runsWhitePatch
    ...
    for k = 1:9
        if k ~= 3
            ...
            [~, measuredSpectraRaw, ~] = evaluateColorchart(filename, allowRoiSelection, patchIndexes, 'raw');
            whitePatchRaw(ind, :) = pads(squeeze(measuredSpectraRaw(:, 2)));
            
            [~, measuredSpectra, ~] = evaluateColorchart(filename, allowRoiSelection, patchIndexes);
