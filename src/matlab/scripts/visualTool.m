for k = [20, 19, 18]
    ...
    if k == 20
        ...
        plotFunWrapper(1, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'Knn_unfixed', 'Unfixed');
    ...
    elseif k == 19
        ...
        predictedLabels = [1, 1, 1, 1, 1, 1, 1, 1];
        plotFunWrapper(3, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'Svm_fixed', 'Fixed');

        cancerProb = [1., 1., 1., 1., 1., 1., 0.32505164, 0.32528109];
        predictedLabels = [1, 1, 1, 1, 1, 1, 0, 0];
        plotFunWrapper(1, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'Knn_fixed', 'Fixed');

        cancerProb = [0.56, 0.62, 0.57, 0.64, 0.58, 0.54, 0.58, 0.64];
        predictedLabels = [1, 1, 1, 1, 1, 1, 1, 1];
        plotFunWrapper(2, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'RF_fixed', 'Fixed');

        trueLabels = [1, 1, 1, 1, 1, 1, 0, 0];
        plotFunWrapper(2, @plotVisualResultAfterClassification, [0, 0, 0, 0, 0, 0, 0, 0], trueLabels, coordinates, orderedRois, segmentMasks, sRGB, 'GD_fixed', 'Fixed');
    ...
    elseif k == 18
        ...
        plotFunWrapper(3, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'Svm_sectioned', 'Sectioned');

        cancerProb = [0.69429197, 0.66705237, 0.39345565, 1., 0.31506872, 1., 0.34356235, 0.66886696];
        predictedLabels = [1, 1, 0, 1, 0, 1, 0, 1];
        plotFunWrapper(1, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'Knn_sectioned', 'Sectioned');

        cancerProb = [0.52, 0.56, 0.52, 0.58, 0.61, 0.53, 0.51, 0.44];
        predictedLabels = [1, 1, 1, 1, 1, 1, 1, 0];
        plotFunWrapper(2, @plotVisualResultAfterClassification, cancerProb, predictedLabels, coordinates, orderedRois, segmentMasks, sRGB, 'RF_sectioned', 'Sectioned');

        trueLabels = [1, 1, 1, 1, 1, 1, 0, 0];
        plotFunWrapper(2, @plotVisualResultAfterClassification, [0, 0, 0, 0, 0, 0, 0, 0], trueLabels, coordinates, orderedRois, segmentMasks, sRGB, 'GD_sectioned', 'Sectioned');
