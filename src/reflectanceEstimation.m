...
switch smoothingMatrixMethod
    ...
    case 'KCor all data' % average xcorr of all measured data 
        M = m.M_measured_xcorr';                   

    case 'KCor macbeth' % average xcorr of all macbeth data 
        M = m.M_measured_xcorr_macbeth;                         

    case 'KCor all specimen' % average xcorr of all measured data for each sample 
    ...
    case 'KCor same malignancy' % average xcorr of all measured cancerous data for cancerous sample
...
if strcmp( noiseType, 'white gaussian')       % signal independent noise 
    Kn = diag( randn(1,length(coeff)) .* options.noiseOrder); %different at every channel
    
elseif strcmp( noiseType, 'independent')
...
elseif strcmp( noiseType, 'givenSNR')
