function [MSI, whiteReference, darkReference, patchMask, maskI] = readMSI(files, x, y, width, height, options, fc)
...
% height: height of image box to be read (y axis)
    ...
    [patchMask, maskI] = makeMasks(files{1+extraImages}, x, y, width, height);
    MSIbands = length(files) - extraImages;
    if (modeAll)
        MSI = zeros(MSIbands, size(maskI,1), size(maskI,2), 3);
    else
        MSI = zeros(MSIbands, height, width, 3);
    end
...
function  [patchMask, maskI] = makeMasks(filedir, x, y, width, height)
