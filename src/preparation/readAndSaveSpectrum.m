...
if ~isempty(strfind(ID(1).Csvid, '.csv'))
    ...
    for i = 1:length(ID)
        ...
        if ~(isempty(measuredSpectrum))
            measuredSpectrumStruct(appendingIdx) = struct('Name', generateName([], 'csv', [], ID(i)), 'Index', appendingIdx, 'Spectrum', measuredSpectrum./white);
...
elseif ~isempty(dir([options.systemdir, '*MeasuredReflectance.mat']))
    ...
    for i = 1:length(ID)
        ...
        measuredSpectrumStruct(appendingIdx) = struct('Name', generateName([], 'csv', [], ID(i)), 'Index', appendingIdx, 'Spectrum', measuredReflectance(idx2).reflectance);
        appendingIdx = length(measuredSpectrumStruct) + 1;
