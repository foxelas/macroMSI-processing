function [ ] = readAndSaveMSI( options,  ID, data, width, height, bands, modeAll)
...
%     save(strcat(savedir, '\ID.mat'), 'ID');
    ...
    for i  = 1:length(ID)
        ...
        [MSI, whiteReference, darkReference] = readMSI({data(ID(i).Data).File}, ID(i).Originx, ID(i).Originy, width, height, bands, modeAll, '..\MATLAB\Cropped\', [ ID(i).Csvid, num2str(ID(i).UniqueCount)]);
         
        name = generateName([], 'imagefile', data(ID(i).Representative), ID(i)); 
